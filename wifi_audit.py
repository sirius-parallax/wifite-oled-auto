...
INTERFACE = "wlan0"
DICTIONARY = "/usr/share/dict/12345678.txt"
LOG_FILE = "/home/%username%/wifite_full_log.txt"
...
OLED_LOG_LINES = 4           # ✅ Сколько строк логов показывать (было 3)
OLED_LOG_MAX_LEN = 21        # ✅ Макс. длина строки (было 20)
    ...
    try:
        with canvas(device) as draw:
            ...
            if mode == 'cracked':
                ...
                # ✅ РЕЖИМ ЛОГОВ: Показывает последние 4 строки
                ...
                # Позиции строк (оптимизировано для 4 строк)
            ...
            # Нижняя панель (всегда) - дата/время + параметры
...
def update_oled_display(scenario_name, current_elapsed, total_elapsed, networks, cracked_net=None, show_logs=False):
    ...
    """
    # 🔴 ПРИОРИТЕТ 1: Новая взломанная сеть
    if cracked_net:
        draw_oled('cracked', {
    ...
    # 🟡 ПРИОРИТЕТ 2: Режим логов
    if show_logs:
        draw_oled('logs', {'logs': recent_logs})
        return
    
    # 🟡 ПРИОРИТЕТ 3: Текущая атака
    if scenario_name:
        target = "Scanning..."
...
def main():
    ...
    print(f"OLED update interval: {OLED_UPDATE_INTERVAL} seconds")
    print(f"OLED log lines: {OLED_LOG_LINES}")
    print("=" * 60)
